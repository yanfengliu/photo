...
Always upgrade the Codex CLI before each review session (defensive against silent model-name rejection and sandbox-policy regressions in older builds; `gpt-5.6-sol` requires Codex CLI ≥ 0.144.1 — older builds reject the model name with `requires a newer version of Codex`): `npm install -g @openai/codex@latest`, then verify with `codex --version`.
...
Every prompt must additionally include the reviewers-must-read directive (quoted in AGENTS.md → Code review) and, where the repo's documentation discipline defines one, its doc-accuracy addendum.
...
Diff reviews take ~5 minutes per CLI on a multi-hundred-line diff. Run both CLIs in parallel with `run_in_background: true`, capturing each CLI's stdout under `tmp/review-runs/<objective>/<date>/<iteration_number>/` (never staged; clean up after synthesis). Wait via a single background `until` poller (`until [ -s codex.txt ] && [ -s claude.txt ]; do sleep 8; done`) so the harness's no-long-sleeps guard doesn't fire and you don't poll repeatedly.

## Reading Codex output

Codex's captured output echoes the entire piped stdin (including the prompt that quotes the BEGIN/END markers as instructions) plus exec-sandbox chatter, then prints the actual review TWICE near the end. A naive `awk` from the first marker captures the PROMPT (which contains the literal marker strings as instructions to Codex), not the review. This was the root cause of the "Codex unreachable" misdiagnosis observed in impls 16–24 of the aoe2 v0.1.6 thread.
