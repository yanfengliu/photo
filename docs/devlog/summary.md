...
- Repeated RAW and SVG Detail loads can reuse a persisted decoded-image cache in the repo-local `decoded-cache` directory with normalized path keys, explicit contract versioning, same-size rewrite validation when metadata changes, failed-write cleanup, and bounded retention, imported RAW and SVG files now start warming that persisted cache in the background right away, and same-session repeat Detail opens can reuse validated full images immediately without blanking while EXIF follow-up continues, including when the user leaves Detail and reopens the same library item.
