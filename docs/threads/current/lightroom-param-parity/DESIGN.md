...
TDD invariants: range table exact; contrast middle-gray invariance for all amounts and identity at 0; saturation −100 → R=G=B exactly and +100 → 2× chroma within gamut; temperature ±100 endpoints 3200/9800 K with direction preserved; mapping functions are the values the GPU uniforms receive (parity test against `viewer.rs` packing); all-extremes `apply_all` stays finite and in-gamut. Visual gate: render `assets/test.jpg` through the CPU pipeline (which is shader-parity-tested) at a grid of representative settings, before vs after the rescale, write PNGs + per-image luminance/chroma metrics to `tmp/param-tuning/`, and inspect — slider values that used to be mid-range must produce visibly moderate results at the same physical strength (new value = old value × range ratio).
