...
## [2026-04-02 03:12 UTC] — Full parameter audit against professional implementations
**Action:** Thorough audit of all 12 adjustment parameters against darktable (toneequal.c, shadhi.c, filmicrgb.c, colorbalancergb.c, sigmoid.c, hazeremoval.c), RawTherapee (ipshadowshighlights.cc, ipvibrance.cc, iplocalcontrast.cc), and GPUImage source code. Three changes made based on findings:
1. Unified all four tone zone sliders to ±2.0 stops (was 1.5 for H/S, 2.5 for W/B). Matches darktable tone equalizer's exact ±2 stop clamp (correction factor 0.25x to 4.0x).
2. Widened whites/blacks slider range from ±50 to ±100 (matching highlights/shadows). Previous ±50 range divided by 100 gave only ±0.5 input to shader, making the 2.5 stop multiplier yield only 1.25 effective stops.
3. Replaced vibrance linear weighting `1 + amt*(1-sat)` with darktable's power-law approach from colorbalancergb.c: `attenuation = 1 - pow(sat, |amount|)`. Gives smoother rolloff for already-saturated colors.
**Result:** Success — 70 tests pass. Release build compiles. Full mapping now: slider ±100 → /100 → ±1.0 → ×2.0 stops = ±2 stops max for all four zone controls.
**Files changed:** `src/edit.rs`, `src/main.rs`, `assets/shaders/image.wgsl`
**Reasoning:** Audit against professional source code revealed: (a) darktable tone equalizer clamps to exactly 0.25x-4.0x (±2 stops) — our 1.5 was too conservative; (b) whites/blacks had a hidden bug — slider range ±50 divided by 100 gave only ±0.5, so even with 2.5x multiplier the max was 1.25 stops; (c) darktable colorbalancergb.c uses `pow(chroma, |vibrance|)` not linear `(1-sat)` — power-law gives better perceptual result on already-vivid colors.
**Notes:** Parameters confirmed correct against sources: exposure (standard 2^EV), saturation (matches GPUImage/common GLSL exactly), contrast sigmoid (k=4..12 reasonable, pros use varied approaches), clarity formula (matches RawTherapee pattern), dehaze (matches He et al. CVPR 2009 recovery equation), Bradford CAT matrix (exact ICC v4 values), CIE daylight locus (exact CIE standard polynomials), tint chromaticity shift (arguably better than darktable's crude Y-division which they annotate "This is baaad!").

## [2026-04-01 20:46, UTC+8] — Comprehensive formula correctness audit via 5 independent review agents
**Action:** Dispatched 5 independent contrarian review agents to audit all parameter tuning formulas: (1) GPU/CPU parity, (2) normalization chain, (3) zone tone math, (4) contrast/vibrance/saturation, (5) temperature/tint/Bradford CAT. Consolidated findings and fixed all identified bugs:
1. **CRITICAL: Bradford matrix row/column transpose** — WGSL `mat3x3(v0,v1,v2)` interprets vectors as columns, but viewer.rs was passing rows. GPU preview and CPU save produced different white balance. Fixed by transposing when packing uniforms.
2. **CRITICAL: Combined tone zone stops unclamped** — Highlights+whites both at max gave ±4 stops (16x multiplier), not the claimed ±2. Added `clamp(-2.0, 2.0)` on total stops in both GPU and CPU.
3. **SIGNIFICANT: Lens vignetting missing from CPU save** — GPU applied vignetting correction but `apply_all()` did not. Added `apply_vignetting()` function and wired it through `save_edited_image()` with UV coordinates and lens coefficients.
4. **SIGNIFICANT: Negative vibrance inverted** — `1 - pow(sat, |amount|)` for negative amounts desaturated muted colors MORE than vivid (opposite of Lightroom). Fixed by using `pow(sat, e)` when amount < 0 (targets saturated colors for desaturation).
5. **SIGNIFICANT: Contrast broke for lum > 1** — Sigmoid bounded (0,1) caused wrong-direction adjustments after exposure boost. Fixed by normalizing HDR luminance into [0,1] before sigmoid, then scaling back.
6. **SIGNIFICANT: Highlights zone was a plateau** — `smoothstep(0.35, 0.75)` had no upper rolloff, making highlights redundant with whites above L_p=0.75. Reshaped to bell: `smoothstep(0.35, 0.55) * (1 - smoothstep(0.75, 1.0))`.
7. **MODERATE: Shadows zone too wide** — Falloff extended to L_p=0.65, affecting midtones. Tightened to 0.25-0.50.
**Result:** Success — 75 tests pass (6 new tests added), release build compiles.
...
## [2026-04-03 23:11, UTC+8] — App state integration + sidebar UI (Task 2)
**Action:** Wired collection module into `src/main.rs` UI layer. Added new imports (`MouseArea`, `Space`, `mouse`, `Point`). Created `ContextMenuKind`, `ContextMenu`, and `DragState` types. Added 11 new fields to `App` struct for collection state management. Added 19 new `Message` variants (all stubbed to `Task::none()`). Added cursor tracking in `handle_event()` via `mouse::Event::CursorMoved`. Built `collection_sidebar()` method rendering a "COLLECTIONS" header with "+" button and scrollable list of collection names with photo counts, hover/right-click support via `MouseArea`. Added `sidebar_item_style` and `sidebar_item_drop_target_style`. Integrated sidebar into `library_view()` as a left panel with divider. Updated `ARCHITECTURE.md` with collection module component, boundary rules, serde dependency, and drift log entry.
**Result:** Success — zero warnings, release build compiles, all 89 tests pass.
**Files changed:** `src/main.rs`, `src/collection.rs` (dead_code allow), `docs/ARCHITECTURE.md`
**Reasoning:** Task 2 of the collections feature; forward-declares all types and messages needed by Tasks 3-8 while keeping them stubbed. Sidebar renders immediately in Library view. Used `MouseArea::new()` instead of `mouse_area()` function since iced 0.13 only exports the struct form.
**Notes:** `Point` import is unused in this task but needed by later tasks (drag-and-drop). All new Message variants are matched in a single catch-all arm returning `Task::none()`. The `#[allow(dead_code)]` annotations on forward-declared types will be removed as later tasks consume them.

## [2026-04-04 06:54, UTC] — Collection CRUD and context menu system (Tasks 3 & 4)
**Action:** Replaced 19 stubbed message handlers with real implementations for collection CRUD and context menu system. Implemented: CreateCollection (auto-name + inline rename mode), CollectionNameChanged/Submit/Cancel (inline editing), SidebarCollectionClicked (double-click to open), SidebarCollectionRightClicked (open context menu at cursor), SidebarCollectionHovered/ThumbnailHovered (hover state), DismissContextMenu, ContextMenuRename (enter rename mode from context menu), ContextMenuDelete (delete with active_collection adjustment). Updated Escape key handler with priority chain: context menu > inline edit > collection nav > active collection > detail view. Added context_menu_overlay() method with positioned overlay using iced Stack widget, supporting SidebarCollection (rename/delete), LibraryPhoto (add to collections), and CollectionPhoto (remove) menu kinds. Added context_menu_item(), context_menu_container_style(), context_menu_button_style() free functions. Added drag_overlay() stub. Updated view() to layer overlays via iced::widget::Stack. Added mouse button pressed/released placeholders in handle_event. Removed #[allow(dead_code)] from active_collection, context_menu, last_collection_click, collection_nav fields. Added 8 new tests covering collection creation, rename, empty-name guard, delete with active-collection adjustment, context menu construction, and double-click detection for sidebar.
**Result:** Success — 97 tests pass (89 existing + 8 new), release build compiles cleanly.
**Files changed:** `src/main.rs`, `docs/devlog-detailed.md`
**Reasoning:** Tasks 3 and 4 of collections feature — implements the core CRUD interactions and the overlay system needed by all context menus. Used `impl Into<String>` for context_menu_item to avoid lifetime issues with format! temporaries. Used `iced::widget::Stack::with_children()` which accepts a Vec of elements.
**Notes:** Remaining stubs (ExitCollectionView, CollectionPhotoClicked/RightClicked, AddPhotoToCollection, RemovePhotoFromCollection, LibraryPhotoRightClicked, TogglePhotoInCollection, ExitCollectionDetail) are left for Tasks 5-8. The `context_menu_item` returns `Element<'static, Message>` to avoid borrowing format! temporaries.

## [2026-04-04 00:11, PDT] — Collection grid view and detail navigation (Tasks 5 & 6)
**Action:** Implemented collection grid view and detail navigation from collection. Added `collection_grid_view()` method rendering a back button, collection title, photo thumbnail grid (6 columns, 150px cards with right-click context menu), and status bar. Routed `library_view()` to collection grid when `active_collection` is set. Replaced 5 stub handlers: ExitCollectionView (clears active_collection), CollectionPhotoClicked (double-click opens Detail with collection_nav set), CollectionPhotoRightClicked (opens CollectionPhoto context menu), RemovePhotoFromCollection (handles both CollectionPhoto and LibraryPhoto context variants), ExitCollectionDetail (returns to Library tab keeping active_collection). Updated arrow key navigation (ArrowRight/Space and ArrowLeft/Backspace) to check `collection_nav` first for collection-scoped cycling. Updated tab bar back button to use ExitCollectionDetail when in collection detail mode. Updated status_bar() name and position display with collection_nav branches. Added 7 new tests (104 total): nav wrap-around, exit handlers, remove photo, double-click sets collection_nav, status bar position format.
**Result:** Success — 104 tests pass, release build compiles with zero warnings.
**Files changed:** `src/main.rs`, `docs/devlog-detailed.md`
