...
| 2026-04-30 | Doc audit: README Features lacked editing/RAW/collections; ARCHITECTURE shader description missed temperature/tint and the renamed `image25` (image 0.25) dependency required by `rawler` 0.7; AGENTS.md referenced unused targets (`docs/api-reference.md`, README "Feature Overview"/"Public Surface") that do not exist in this repo. Recorded but not fully resolved: `docs/changelog.md`, `docs/threads/` and `Cargo.toml` semver bumps mandated by AGENTS.md are not yet established (Cargo version is still `0.1.0`). | claude (doc audit) |
